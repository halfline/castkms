					    ...
					    ktime_t now, unsigned int rate)
{
	...
	return div_u64(elapsed_ns * rate, NSEC_PER_SEC);
}
...
static enum hrtimer_restart castkms_audio_timer(struct hrtimer *timer)
{
	...
	snd_pcm_period_elapsed(rt->output->active_substream);
...
static int castkms_pcm_open(struct snd_pcm_substream *substream)
{
	 ...
	 */
	snd_pcm_hw_constraint_eld(substream->runtime, out->eld);
				     ...
				     struct snd_pcm_audio_tstamp_report *audio_tstamp_report)
{
	...
	*audio_ts = ns_to_timespec64(
		div_u64(frames * NSEC_PER_SEC, runtime->rate));
			      ...
			      struct drm_connector *connector)
{
	...
	if (changed) {
		snd_ctl_notify_one(out->castkmsdev->audio->card,
...
void castkms_audio_cleanup(struct castkms_device *castkmsdev)
{
	...
	for (i = 0; i < audio->num_outputs; i++) {
		struct castkms_audio_output *out = &audio->outputs[i];
